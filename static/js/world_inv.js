...
<div id="invModal" style="position:fixed;inset:0;z-index:120;display:none">
  ...
  <div class="inv-sheet">
    ...
    <div id="invStats" class="inv-stats">
      ...
      <div class="row"><span>Нагрузка</span><span><span id="invLoad">0</span>% · <span id="invLoadTierModal">—</span></span></div>
      <div class="row"><span>Эффекты</span><span>скорость ×<span id="invSpeedMul">1.00</span>, усталость ×<span id="invFatigueMul">1.00</span></span></div>
      <div class="row"><span>Ячеек</span><span><span id="invStacks">0</span> стопок · <span id="invPieces">0</span> предметов</span></div>
  ...
  function render(data){
    ...
    const tierEl = $('invLoadTierModal');
    const spdEl = $('invSpeedMul');
    const fatMulEl = $('invFatigueMul');
    if(!list) return;
    ...
    if (tierEl) tierEl.textContent = totals.tier_desc || '—';
    if (spdEl) spdEl.textContent = Number(totals.speed_mul || 1).toFixed(2);
    if (fatMulEl) fatMulEl.textContent = Number(totals.fatigue_mul || 1).toFixed(2);
